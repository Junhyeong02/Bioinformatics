...
def main(input_path:str, output_path:str, qcut:int, cpu:int, cut_ratio:float, pair_type:str):   
    print(input_path)
    ...
    for cnt, sample in enumerate(sample_list):
        ...
        if pair_type == "SE":
            ...
            command = "fastp -i {} -o {} -l {} -q {} -w {} -h {} -j {} {}".format(sample, out_file, len_cut, qcut, cpu, html_file, json_file, phred)
        ...
        ### Paired end
            ...
            command = "fastp -i {} -I {} -o {} -O {} -l {} -c --detect_adapter_for_pe -q {} -w {} -h {} -j {} {}".format(file1, file2, out_file1, out_file2, int(len_cut), qcut, cpu, html_file, json_file, phred)
        
        p = subprocess.Popen(command, shell = True)
        # p = subprocess.Popen('echo "{}"'.format(command), shell = True)
        process.append(p)
...
if __name__ == "__main__":    
    ...
    main(input_path, output_path, qcut, CPU, cut_ratio, pair_type)
